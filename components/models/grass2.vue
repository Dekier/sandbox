...
if (nodes.grass003) {
  const geometry = nodes.grass003.geometry;
  const material = nodes.grass003.material!.clone();
  const positionY = nodes.grass003.position.y;
  ...
  for (let i = 0; i < instancesCount.value; i++) {
    ...
    const randomScale = Math.random() * 0.6 + 1;
